...
// modified from https://github.com/volatiletech/sqlboiler/blob/master/drivers/sqlboiler-sqlite3/driver/sqlite3.go
...
// Assemble the db info
func (s SQLiteDriver) Assemble(config drivers.Config) (dbinfo *structs.DBInfo, err error) {
	defer func() {
...
func (s SQLiteDriver) Columns(schema string, table *structs.Table, tableName string, whitelist, blacklist []string) ([]structs.Column, error) {
...
func (s SQLiteDriver) PrimaryKeyInfo(schema, tableName string) (*structs.PrimaryKey, error) {
	...
	var pk *structs.PrimaryKey
	if len(columns) > 0 {
		pk = &structs.PrimaryKey{Columns: columns}
	}
	return pk, nil
}

// ForeignKeyInfo retrieves the foreign keys for a given table name.
func (s SQLiteDriver) ForeignKeyInfo(schema, tableName string) ([]structs.ForeignKey, error) {
	dbForeignKeys, err := s.foreignKeyInfoFromDB(schema, tableName)
	if err != nil {
		return nil, fmt.Errorf("read foreign keys info from db: %w", err)
	}

	return dbForeignKeys, nil
}
func (s SQLiteDriver) foreignKeyInfoFromDB(schema, tableName string) ([]structs.ForeignKey, error) {
...
func (SQLiteDriver) TranslateColumnType(c structs.Column) structs.Column {
	if c.Nullable {
